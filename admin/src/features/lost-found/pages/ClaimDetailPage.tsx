...
import { ClaimStatusBadge, LinkToItemModal, LostFoundStatusBadge } from "../components";

// ─── Config ─────────────────────────────────────────────────────────────────

const CLAIM_STATUS_CONFIG: Record<
  ClaimStatus,
  { accent: string; light: string; border: string; text: string; Icon: IconType; label: string }
> = {
  [ClaimStatus.PENDING]: {
    accent: "#f59e0b", light: "bg-amber-50", border: "border-amber-200", text: "text-amber-700",
    Icon: LuClock, label: "Pending Review",
  },
  [ClaimStatus.APPROVED]: {
    accent: "#3b82f6", light: "bg-blue-50", border: "border-blue-200", text: "text-blue-700",
    Icon: LuCheck, label: "Approved",
  },
  [ClaimStatus.REJECTED]: {
    accent: "#ef4444", light: "bg-red-50", border: "border-red-200", text: "text-red-600",
    Icon: LuX, label: "Rejected",
  },
  [ClaimStatus.COMPLETED]: {
    accent: "#10b981", light: "bg-emerald-50", border: "border-emerald-200", text: "text-emerald-700",
    Icon: LuCircleCheck, label: "Completed",
  },
};

const CATEGORY_CONFIG: Record<LostFoundCategory, { label: string; Icon: IconType }> = {
...
> = {
  [ClaimStatus.PENDING]: {
    label: "Undo", confirmLabel: "Confirm Undo",
    border: "border-gray-200", bg: "bg-gray-50/60", titleColor: "text-gray-600",
    confirmBtn: "bg-gray-700 hover:bg-gray-800 text-white",
    placeholder: "Reason for reverting this claim (optional)...",
  },
  [ClaimStatus.APPROVED]: {
    label: "Approve", confirmLabel: "Confirm Approval",
    border: "border-primary/20", bg: "bg-primary/[0.04]", titleColor: "text-primary",
    confirmBtn: "bg-primary hover:bg-primary-600 text-white",
    placeholder: "Add a note for the claimant or internal record (optional)...",
  },
  [ClaimStatus.REJECTED]: {
    label: "Reject", confirmLabel: "Confirm Rejection",
    border: "border-red-200", bg: "bg-red-50/50", titleColor: "text-red-600",
    confirmBtn: "bg-red-600 hover:bg-red-700 text-white",
    placeholder: "Explain why this claim is rejected (optional, visible to staff)...",
  },
  [ClaimStatus.COMPLETED]: {
    label: "Mark Returned", confirmLabel: "Confirm Return",
    border: "border-emerald-200", bg: "bg-emerald-50/50", titleColor: "text-emerald-700",
...
function InfoField({ icon: Icon, label, value }: { icon: IconType; label: string; value?: string | null }) {
...
export function ClaimDetailPage() {
  ...
  const { data: claim, isLoading, isError } = useFetchOneClaimQuery(id!, { skip: !id });
  ...
  const confirmAction = async () => {
    ...
    try {
      await reviewClaim({
        claimId: id,
        body: { status: pendingAction, staffNote: actionNote.trim() || undefined },
  ...
  const handleUnlink = async () => {
    if (!id) return;
    const r = await alertService.confirmModal("Unlink Claim", "This will remove the link between this claim and the lost & found item. Continue?");
  ...
  const baseBreadcrumb = [
    { label: "Support" },
    { label: "Lost & Found", path: "/support/lost-found?tab=claims", icon: LuClipboardList },
  ];
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-1 text-sm text-gray-400">The claim may have been deleted.</p>
          <button
            type="button"
            onClick={() => navigate("/support/lost-found?tab=claims")}
            className="mt-4 flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm text-white hover:bg-primary-600"
          >
  ...
  const hasItemDesc = claim.itemDescription || claim.lostLocation || claim.lostDate;
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2.5">
            <h1 className="text-lg font-bold text-gray-900">{claim.referenceCode}</h1>
            <ClaimStatusBadge status={claim.status} />
          </div>
          <p className="text-xs text-gray-500">
            Submitted {formatDateTime(claim.createdAt)}
            {claim.reviewedAt && claim.reviewedBy && (
              <> &middot; Reviewed by <span className="font-medium text-gray-600">{claim.reviewedBy.name}</span> {formatDateTime(claim.reviewedAt)}</>
            )}
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div className="px-6 pt-5 pb-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <InfoField icon={LuUser} label="Name" value={claim.claimantName} />
              <InfoField icon={LuMail} label="Email" value={claim.claimantEmail} />
              <InfoField icon={LuPhone} label="Phone" value={claim.claimantPhone} />
              <InfoField icon={LuPlane} label="Flight" value={
                claim.flightNumber
                ...
                <div className="space-y-2.5">
                  {categoryCfg && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-700">{categoryCfg.label}</span>
                    </div>
                  )}
                  {claim.itemDescription && (
                    <p className="text-sm leading-relaxed text-gray-700">{claim.itemDescription}</p>
                  )}
            ...
            {claim.lostFound ? (
              <div className="flex items-center justify-between gap-3">
                <Link
                  ...
                  className="flex min-w-0 items-center gap-3 rounded-lg border border-gray-100 px-3 py-2.5 transition-colors hover:border-primary/30 hover:bg-primary/[0.02]"
                >
                  <LuLink className="h-4 w-4 shrink-0 text-primary" />
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-gray-900">
                      {claim.lostFound.displayNames?.en || claim.lostFound.displayNames?.lo || "Untitled"}
                    </p>
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="font-mono text-[11px] text-gray-400">{claim.lostFound.referenceCode}</span>
              ...
              <div className="flex items-center gap-3 rounded-lg border border-dashed border-gray-200 bg-gray-50/50 px-4 py-3">
                ...
                <p className="flex-1 text-sm text-gray-400">Not linked to any item</p>
                {canUpdate && (
                  <button
                    type="button"
                    onClick={() => setLinkModalOpen(true)}
                    className="flex shrink-0 items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white transition hover:bg-primary-600"
                  >
          ...
          <div className="px-6 py-5">
            ...
            {editingNote ? (
                ...
                <div className="mt-1 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={saveNote} disabled={isReviewing}
                      ...
                      {isReviewing ? <LuRefreshCw className="h-3 w-3 animate-spin" /> : <LuSave className="h-3 w-3" />}
                      Save
                    </button>
                    ...
                    {claim.staffNote && (
                      <button type="button" onClick={() => setEditNote("")}
                        className="ml-1 text-xs text-gray-400 hover:text-red-500" title="Clear note">
              ...
              <div className="group flex items-start justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50/80 px-3.5 py-3">
                <p className="text-sm leading-relaxed text-blue-800">{claim.staffNote}</p>
                {canUpdate && (
                  <button type="button" onClick={openNoteEditor} disabled={isReviewing} title="Edit note"
                    className="shrink-0 rounded-md p-1 text-blue-400 opacity-0 transition-all hover:bg-blue-100 hover:text-blue-600 group-hover:opacity-100 disabled:opacity-0">
              ...
              canUpdate ? (
                <button type="button" onClick={openNoteEditor} disabled={isReviewing}
                  ...
                  <LuPlus className="h-3.5 w-3.5" />Add note
                </button>
              ) : (
              ...
              <div className="bg-gray-50/40 px-6 py-5">
                ...
                {claim.status === ClaimStatus.PENDING && !pendingAction && (
                  <div className="flex gap-2">
                    <button type="button" onClick={() => openAction(ClaimStatus.APPROVED)} disabled={isReviewing}
                      className="flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-600 disabled:opacity-50">
                ...
                {claim.status === ClaimStatus.APPROVED && !pendingAction && (
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => openAction(ClaimStatus.COMPLETED)} disabled={isReviewing}
                      ...
                      <LuCircleCheck className="h-4 w-4" />Mark Returned
                    </button>
                    <button type="button" onClick={() => openAction(ClaimStatus.PENDING)} disabled={isReviewing}
                      ...
                      <LuRotateCcw className="h-4 w-4" />Undo
                    </button>
                  </div>
                )}
                {claim.status === ClaimStatus.REJECTED && !pendingAction && (
                  <div>
                    <button type="button" onClick={() => openAction(ClaimStatus.PENDING)} disabled={isReviewing}
                      className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 disabled:opacity-50">
                      <LuRotateCcw className="h-4 w-4" />Undo Rejection
                    </button>
                  </div>
                )}
                ...
                >
                  <div className="overflow-hidden">
                    {actionCfg && (
                      <div className={cn("mt-4 rounded-lg border px-4 py-4", actionCfg.border, actionCfg.bg)}>
                        <p className={cn("mb-2.5 flex items-center gap-1.5 text-xs font-semibold", actionCfg.titleColor)}>
                          {pendingAction === ClaimStatus.PENDING && <LuRotateCcw className="h-3.5 w-3.5" />}
                          {pendingAction === ClaimStatus.APPROVED && <LuCheck className="h-3.5 w-3.5" />}
                          {pendingAction === ClaimStatus.REJECTED && <LuX className="h-3.5 w-3.5" />}
                          {pendingAction === ClaimStatus.COMPLETED && <LuCircleCheck className="h-3.5 w-3.5" />}
                          {actionCfg.label}
                          <span className="font-normal opacity-60">&mdash; add a note (optional)</span>
                        </p>
                        <textarea
                          ref={actionNoteRef}
                          value={actionNote}
                        ...
                        <div className="mt-1 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={confirmAction} disabled={isReviewing}
                              className={cn("flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition disabled:opacity-50", actionCfg.confirmBtn)}>
                              {isReviewing ? <LuRefreshCw className="h-3.5 w-3.5 animate-spin" /> : <LuCheck className="h-3.5 w-3.5" />}
                              {actionCfg.confirmLabel}
                            </button>
                            <button type="button" onClick={cancelAction} disabled={isReviewing}
                          ...
                          <span className="text-[10px] tabular-nums text-gray-300">{actionNote.length}/1000</span>
                        </div>
                      </div>
                    )}
                  </div>
